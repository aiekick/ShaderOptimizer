    ...
    if (isFocused) {
        ...
        if (ctrlPressed) {
            ...
            if (ImGui::IsKeyDown(ImGuiKey_V)) {
                if (onChangeFunctor != nullptr) {
                    onChangeFunctor(m_Editor.GetText());
                }
            }
        }
    }
...
std::string CodeEditor::GetCode() {
    return m_Editor.GetText();
}

void CodeEditor::registerOnChangeEvent(OnChangeFunctor vFunctor) {
    onChangeFunctor = vFunctor;
}

void CodeEditor::registerOnFocusedEvent(OnFocusedFunctor vFunctor) {
    onFocusedFunctor = vFunctor;
}

void CodeEditor::registerOnShowInFolderViewEvent(OnShowInFolderViewFunctor vFunctor) {
    onShowInFolderViewFunctor = vFunctor;
}
