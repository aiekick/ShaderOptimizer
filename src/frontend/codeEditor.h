...
class IMGUI_API CodeEditor {
public:
    typedef std::function<void(int folderViewId)> OnFocusedFunctor;
    typedef std::function<void(const std::string& filePath, int folderViewId)> OnShowInFolderViewFunctor;
    typedef std::function<void(std::string)> OnChangeFunctor;

private:
    OnChangeFunctor onChangeFunctor{};
    OnFocusedFunctor onFocusedFunctor{};
    OnShowInFolderViewFunctor onShowInFolderViewFunctor{};
...
public:
    ...
    std::string GetCode();
    void registerOnChangeEvent(OnChangeFunctor vFunctor);
    void registerOnFocusedEvent(OnFocusedFunctor vFunctor);
    void registerOnShowInFolderViewEvent(OnShowInFolderViewFunctor vFunctor);
