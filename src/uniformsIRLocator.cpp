...
bool TUniformsIRLocator::visitBinary(glslang::TVisit /* visit */, glslang::TIntermBinary* vNode) {
    if (vNode->getOp() == glslang::EOpIndexDirectStruct) {
        bool reference = vNode->getLeft()->getType().isReference();
        const glslang::TTypeList* members = reference ? vNode->getLeft()->getType().getReferentType()->getStruct() : vNode->getLeft()->getType().getStruct();
...
void TUniformsIRLocator::visitSymbol(glslang::TIntermSymbol* vNode) {
    // we will get the samplers name
    if (vNode->getBasicType() == glslang::TBasicType::EbtSampler) {
